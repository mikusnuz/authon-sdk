<template>
  <div class="page" style="max-width: 640px">
    ...
    <div v-if="statusLoading" class="page-centered">
      <div class="loading-spinner-lg" />
    </div>

    <template v-else>
      <div v-if="localError" class="alert alert-error" style="margin-bottom: 16px">{{ localError }}</div>
      ...
      <div class="card" style="margin-bottom: 20px">
        ...
        <template v-if="!status?.enabled && setupStep === 'idle'">
          <button class="btn btn-primary" @click="handleSetupStart" :disabled="actionLoading">
            <span v-if="actionLoading" class="loading-spinner" />
...
const loadStatus = async () => {
  ...
  try {
    status.value = await authon.client.getMfaStatus()
  } catch {
    status.value = null
  } finally {
...
const handleSetupStart = async () => {
  ...
  try {
    const data = await authon.client.setupMfa()
    qrData.value = { secret: data.secret, qrCodeSvg: data.qrCodeSvg, backupCodes: data.backupCodes }
    setupStep.value = 'qr'
  } catch (err: any) {
...
const handleDisable = async () => {
  ...
  actionLoading.value = true
  try {
    await authon.client.disableMfa(disableCode.value)
...
const handleRegenerate = async () => {
  ...
  actionLoading.value = true
  try {
    const codes = await authon.client.regenerateBackupCodes(regenCode.value)
