<template>
  <div class="page" style="max-width: 720px">
    ...
    <div v-if="fetching" class="page-centered">
      <div class="loading-spinner-lg" />
    </div>

    <template v-else>
      <div v-if="loadError" class="alert alert-error" style="margin-bottom: 16px">{{ loadError }}</div>
...
const load = async () => {
  ...
  loadError.value = ''
  try {
    const data = await authon.client.listSessions()
...
const handleRevoke = async (sessionId: string) => {
  ...
  try {
    ...
    sessions.value = sessions.value.filter((s: SessionInfo) => s.id !== sessionId)
    successMsg.value = 'Session revoked successfully.'
  } catch (err: any) {
    loadError.value = err instanceof Error ? err.message : 'Failed to revoke session'
  } finally {
    revoking.value = null
  }
}
