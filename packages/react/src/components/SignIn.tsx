  ...
  return (
    <div style={cardStyle}>
      <div style={logoStyle}>
        ...
        {branding.brandName && (
          <p style={subtitleStyle}>to {branding.brandName}</p>
        )}
      </div>
      ...
      {branding.termsUrl || branding.privacyUrl ? (
        <div style={{ textAlign: 'center', fontSize: 11, color: theme.textMuted, marginTop: -8 }}>
          {branding.termsUrl && (
            <a href={branding.termsUrl} target="_blank" rel="noopener noreferrer" style={{ color: theme.textMuted }}>Terms</a>
          )}
          {branding.termsUrl && branding.privacyUrl && ' · '}
          {branding.privacyUrl && (
            <a href={branding.privacyUrl} target="_blank" rel="noopener noreferrer" style={{ color: theme.textMuted }}>Privacy</a>
          )}
        </div>
      ) : null}
    </div>
...
function OAuthButton({ provider, loading, disabled, onClick, borderRadius }: OAuthButtonProps) {
...
function CompactOAuthButton({ provider, loading, disabled, onClick, borderRadius }: OAuthButtonProps) {
  const [hovered, setHovered] = useState(false);
  const colors = PROVIDER_COLORS[provider] ?? { bg: '#333', text: '#fff' };
  const name = PROVIDER_DISPLAY_NAMES[provider] ?? provider;
  const config = getProviderButtonConfig(provider);
