# @authon/nextjs

## 0.8.1

- Document safe BFF refresh rotation and targeted provider-session revocation.

## 0.8.0

### Minor Changes

- Add BFF-safe Authorization Code + PKCE authentication, browser-storage-free JS mode, and Next.js server transaction/exchange/revoke helpers.

### Patch Changes

- Updated dependencies []:
  - @authon/shared@0.8.0
  - @authon/react@0.8.0

## 0.7.17

### Patch Changes

- No Next.js runtime API changes. This release keeps the fixed Authon SDK version group synchronized.

- Updated dependencies:
  - @authon/shared@0.7.17
  - @authon/react@0.7.17

## 0.7.16

### Patch Changes

- No Next.js runtime API changes. This release keeps the fixed Authon SDK version group synchronized.

- Updated dependencies:
  - @authon/shared@0.7.16
  - @authon/react@0.7.16

## 0.7.15

### Patch Changes

- Add validated server response parsing, opt-in fail-closed remote verification, secure request-header middleware propagation, and a cookie-synchronized Next.js provider.

- Updated dependencies [[`caec43c`](https://github.com/mikusnuz/authon-sdk/commit/caec43cb40e0e0f173201723bbc31affce42b48d)]:
  - @authon/shared@0.7.15
  - @authon/react@0.7.15

## 0.1.17

### Patch Changes

- Add provider-level OAuth flow support (auto/popup/redirect), redirect handoff recovery, and framework session-restore alignment.

- Updated dependencies []:
  - @authon/shared@0.1.17
  - @authon/react@0.1.17
