import type { BrandingConfig, OAuthProviderType } from '@authup/shared';
import { DEFAULT_BRANDING } from '@authup/shared';
...
export class ModalRenderer {
  ...
  private render(view: 'signIn' | 'signUp'): void {
    ...
    host.setAttribute('data-authup-modal', '');
  ...
  private buildCSS(): string {
    ...
    return `
      :host {
        --authup-primary-start: ${b.primaryColorStart || '#7c3aed'};
        --authup-primary-end: ${b.primaryColorEnd || '#4f46e5'};
        --authup-light-bg: ${b.lightBg || '#ffffff'};
        --authup-light-text: ${b.lightText || '#111827'};
        --authup-dark-bg: ${b.darkBg || '#0f172a'};
        --authup-dark-text: ${b.darkText || '#f1f5f9'};
        --authup-radius: ${b.borderRadius ?? 12}px;
        --authup-font: 'Inter', -apple-system, BlinkMacSystemFont, 'Segoe UI', sans-serif;
        font-family: var(--authup-font);
        color: var(--authup-light-text);
      }
      * { box-sizing: border-box; margin: 0; padding: 0; }
      ...
      .modal-container {
        ...
        background: var(--authup-light-bg);
        border-radius: var(--authup-radius);
        padding: 32px;
      ...
      .provider-btn {
        ...
        width: 100%; padding: 10px 16px; border-radius: calc(var(--authup-radius) * 0.67);
      ...
      .input {
        ...
        border: 1px solid #d1d5db; border-radius: calc(var(--authup-radius) * 0.5);
        font-size: 14px; font-family: var(--authup-font);
        outline: none; transition: border-color 0.15s;
      }
      .input:focus { border-color: var(--authup-primary-start); box-shadow: 0 0 0 3px rgba(124,58,237,0.1); }
      .submit-btn {
        width: 100%; padding: 10px;
        background: linear-gradient(135deg, var(--authup-primary-start), var(--authup-primary-end));
        color: #fff; border: none; border-radius: calc(var(--authup-radius) * 0.5);
        font-size: 14px; font-weight: 600; cursor: pointer;
        font-family: var(--authup-font); transition: opacity 0.15s;
      }
      .submit-btn:hover { opacity: 0.9; }
      .switch-view { text-align: center; margin-top: 16px; font-size: 13px; color: #6b7280; }
      .switch-view a { color: var(--authup-primary-start); text-decoration: none; font-weight: 500; }
